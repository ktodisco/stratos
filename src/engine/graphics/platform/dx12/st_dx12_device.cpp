...
std::unique_ptr<class st_command_queue> st_dx12_device::create_command_queue(const st_command_queue_desc& desc)
{
	...
	HRESULT result = _d3d_device->CreateCommandQueue(
		&queue_desc,
...
std::unique_ptr<class st_command_list> st_dx12_device::create_command_list(const st_command_list_desc& desc)
{
...
std::unique_ptr<st_swap_chain> st_dx12_device::create_swap_chain(const st_swap_chain_desc& desc)
{
	...
	HRESULT result = _context->get_dxgi_factory()->CreateSwapChainForHwnd(
		command_queue->get(),
...
void st_dx12_device::reconfigure_swap_chain(const st_swap_chain_desc& desc, st_swap_chain* swap_chain_)
{
...
st_texture_view* st_dx12_device::get_backbuffer_view(st_swap_chain* swap_chain_, uint32_t index)
{
	st_dx12_swap_chain* swap_chain = static_cast<st_dx12_swap_chain*>(swap_chain_);
...
{
	return _context->get_dxgi_factory()->IsCurrent() ?
		e_st_swap_chain_status::current :
		e_st_swap_chain_status::out_of_date;
}

std::unique_ptr<st_texture> st_dx12_device::create_texture(const st_texture_desc& desc)
{
	...
	HRESULT result = _d3d_device->CreateCommittedResource(
		&heap_properties,
...
void st_dx12_device::set_texture_name(st_texture* texture_, std::string name)
{
...
std::unique_ptr<st_texture_view> st_dx12_device::create_texture_view(const st_texture_view_desc& desc)
{
	...
	if (desc._usage & e_st_view_usage::render_target)
	{
		if (texture->_usage & e_st_texture_usage::depth_target)
		{
			...
			_d3d_device->CreateDepthStencilView(*texture->_handle.GetAddressOf(), nullptr, dsv_handle._handle);
		...
		else if (texture->_usage & e_st_texture_usage::color_target)
		{
			...
			_d3d_device->CreateRenderTargetView(*texture->_handle.GetAddressOf(), nullptr, rtv_handle._handle);
	...
	else if (desc._usage & e_st_view_usage::shader_resource)
	{
		...
		_d3d_device->CreateShaderResourceView(texture->_handle.Get(), &srv_desc, srv_handle._handle);
	...
	else if (desc._usage & e_st_view_usage::unordered_access)
	{
		...
		_d3d_device->CreateUnorderedAccessView(texture->_handle.Get(), nullptr, &uav_desc, uav_handle._handle);
...
std::unique_ptr<st_sampler> st_dx12_device::create_sampler(const st_sampler_desc& desc)
{
	...
	_d3d_device->CreateSampler(&sampler_desc, sampler_handle._handle);
...
std::unique_ptr<st_buffer> st_dx12_device::create_buffer(const st_buffer_desc& desc)
{
...
void st_dx12_device::create_buffer_internal(size_t size, ID3D12Resource** resource)
{
	D3D12_HEAP_PROPERTIES heap_properties{
...
std::unique_ptr<st_buffer_view> st_dx12_device::create_buffer_view(const st_buffer_view_desc& desc)
{
	...
	if (desc._usage & e_st_view_usage::shader_resource)
	{
		...
		st_dx12_cpu_descriptor_handle srv_handle = _resource_heap->allocate_handle();
		_d3d_device->CreateShaderResourceView(buffer->_buffer.Get(), &srv_desc, srv_handle._handle);
	...
	else if (desc._usage & e_st_view_usage::unordered_access)
	{
		...
		st_dx12_cpu_descriptor_handle uav_handle = _resource_heap->allocate_handle();
		_d3d_device->CreateUnorderedAccessView(buffer->_buffer.Get(), nullptr, &uav_desc, uav_handle._handle);
...
void st_dx12_device::map(st_buffer* buffer_, uint32_t subresource, const st_range& range_, void** out_data)
{
	st_dx12_buffer* buffer = static_cast<st_dx12_buffer*>(buffer_);

	D3D12_RANGE range { range_.begin, range_.end };
	HRESULT result = buffer->_buffer->Map(subresource, &range, out_data);
...
void st_dx12_device::unmap(st_buffer* buffer_, uint32_t subresource, const st_range& range_)
{
	st_dx12_buffer* buffer = static_cast<st_dx12_buffer*>(buffer_);
...
void st_dx12_device::set_buffer_name(st_buffer* buffer_, std::string name)
{
	st_dx12_buffer* buffer = static_cast<st_dx12_buffer*>(buffer_);
	ST_NAME_DX12_OBJECT(buffer->_buffer.Get(), str_to_wstr(name).c_str());
}

std::unique_ptr<st_resource_table> st_dx12_device::create_resource_table()
{
	std::unique_ptr<st_dx12_resource_table> table = std::make_unique<st_dx12_resource_table>();
	return std::move(table);
}

std::unique_ptr<st_resource_table> st_dx12_device::create_resource_table_compute()
{
	std::unique_ptr<st_dx12_resource_table> table = std::make_unique<st_dx12_resource_table>();
	return std::move(table);
}

void st_dx12_device::set_constant_buffers(st_resource_table* table_, uint32_t count, const st_buffer_view** cbs)
{
...
void st_dx12_device::set_textures(
...
void st_dx12_device::set_buffers(st_resource_table* table_, uint32_t count, const st_buffer_view** buffers)
{
	st_dx12_resource_table* table = static_cast<st_dx12_resource_table*>(table_);
...
void st_dx12_device::set_uavs(st_resource_table* table_, uint32_t count, const st_texture_view** textures)
{
	st_dx12_resource_table* table = static_cast<st_dx12_resource_table*>(table_);
...
void st_dx12_device::update_textures(st_resource_table* table_, uint32_t count, const st_texture_view** views)
{
	st_dx12_resource_table* table = static_cast<st_dx12_resource_table*>(table_);
...
std::unique_ptr<st_shader> st_dx12_device::create_shader(const char* filename, e_st_shader_type_flags type)
{
...
std::unique_ptr<st_pipeline> st_dx12_device::create_graphics_pipeline(const st_graphics_state_desc& desc)
{
	...
	_d3d_device->CreateGraphicsPipelineState(
		&pipeline_desc,
...
std::unique_ptr<st_pipeline> st_dx12_device::create_compute_pipeline(const struct st_compute_state_desc& desc)
{
	std::unique_ptr<st_dx12_pipeline> pipeline = std::make_unique<st_dx12_pipeline>();
	...
	_d3d_device->CreateComputePipelineState(
		&pipeline_desc,
		__uuidof(ID3D12PipelineState),
		(void**)pipeline->_pipeline.GetAddressOf());
...
std::unique_ptr<st_vertex_format> st_dx12_device::create_vertex_format(
...
std::unique_ptr<st_render_pass> st_dx12_device::create_render_pass(const st_render_pass_desc& desc)
{
...
std::unique_ptr<st_framebuffer> st_dx12_device::create_framebuffer(const st_framebuffer_desc& desc)
{
