...
st_vk_device::st_vk_device(const st_device_desc& desc, vk::Instance* instance)
	: _instance(instance)
{
...
{
	std::unique_ptr<st_vk_command_allocator> allocator = std::make_unique<st_vk_command_allocator>(desc, &_device, _queue_family_index);
	return std::move(allocator);
}

std::unique_ptr<class st_command_list> st_vk_device::create_command_list(const st_command_list_desc& desc)
{
	std::unique_ptr<st_vk_command_list> command_list = std::make_unique<st_vk_command_list>(desc, this);
...
void st_vk_device::map(st_buffer* buffer_, uint32_t subresource, const st_range& range, void** out_data)
{
	...
	VK_VALIDATE(_device.mapMemory(buffer->_memory, range.begin, size, vk::MemoryMapFlags(0), out_data));
}
