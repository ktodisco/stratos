...
void cs_main(uint3 threadID : SV_DispatchThreadID)
{
	...
	// First, we need the view direction. If 1.0 is straight up and 0.0 straight
	// down, then we need to invert the UV y coordinate and then x is given by
	// the sin and y is given by cos.
	float y = 1.0f - uv.y;
	float2 dir = normalize(float2(sin(y * PI), cos(y * PI)));
	...
	// Either the vector is pointing away from the planet, or there is no intersection.
	if (test < 0.0f || dir.y > 0.0f)
	{
        ...
        for (int i = 0; i < num_samples; ++i)
        {
            ...
            optical_depth_o += max(0.0f, 1.0f - abs(sample_height - constants.ozone_params.w) / 15000.0f) * sample_length;
        }
